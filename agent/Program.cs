...
var heartbeatTask = Task.Run(async () =>
{
    ...
    {
        await Task.Delay(TimeSpan.FromSeconds(5), stop.Token).ContinueWith(_ => { });
        if (ws.State == WebSocketState.Open) await Send(ws, new { type = "agent.heartbeat" });
    }
});
...
try
{
    while (!stop.IsCancellationRequested && ws.State == WebSocketState.Open)
    {
        if (sharing)
        {
            ...
            await ws.SendAsync(frame, WebSocketMessageType.Binary, true, stop.Token);
...
finally
{
    ...
    {
        try { await Send(ws, new { type = "agent.sharing", active = false }); } catch { }
        try { await ws.CloseAsync(WebSocketCloseStatus.NormalClosure, "Agent stopped", CancellationToken.None); } catch { }
    }
    await heartbeatTask;
}
